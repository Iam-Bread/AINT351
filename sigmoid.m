...
plot(uSquError(1,1:200),'c');
plot(uSquError(2,1:200),'m');
title('10580008:Mean Square Error');


%Use forward modle on data different to that the network was trained on
[bP1,bP2] = RevoluteForwardKinematics2D(armLen, data, origin);

%Use the trained weights and the formardpass of the network
net3 = useNetwork(bP2,w1,w2,samples);

%Use forward module on the output of the network
