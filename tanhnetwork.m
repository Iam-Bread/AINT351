...
%randomly generate weight matrix
 std = sqrt(6/5);
%  w1 = (std+std).* randn(nodes,3)+std;
%  w2 = (std+std).* randn(2,nodes+1)+std;
...
for i=1:10000
   ...
   %a2 = (exp(2*net2)-1)./(exp(2*net2)+1);
   
   a2 = (exp(net2)-exp(-net2))./(exp(net2)+exp(-net2));
